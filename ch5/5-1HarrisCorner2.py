...
def onCornerHarris(thresh):  # trackbar를 움직일 때마다 자동으로 호출
    CN = cv.normalize(C, 0, 400, cv.NORM_MINMAX)  # 트랙바는 정수만 다룸. 0~400 사이로 값을 키워줌
    rcorners = []
...
cv.createTrackbar("Threshold", "harris detect", thresh, 30, onCornerHarris)  # gui. trackbar 이름, 붙어있는 창, 바의 현재 값, 가장 큰 값, 호출 함수
