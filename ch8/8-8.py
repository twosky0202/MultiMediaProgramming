...
class DogSpeciesRecognition(QMainWindow):
    ...
    def recognitionFunction(self):
        x = np.reshape(cv.resize(self.img, (224, 224)), (1, 224, 224, 3))  # 1) 데이터 준비

        res = cnn.predict(x)[0]  # 4) 예측
        top5 = np.argsort(-res)[:5]  # Top5 vs Top1 - argmax
        top5_dog_species_names = [dog_species[i] for i in top5]
        for i in range(5):
            prob = '(' + str(res[top5[i]]) + ')'
            name = str(top5_dog_species_names[i]).split('-')[1]
            cv.putText(self.img, prob + name, (10, 100 + i * 30), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
        cv.imshow('Dog image', self.img)
