...
class TrafficWeak(QMainWindow):
    ...
    def signFunction(self):
        ...
        for fname, _ in self.signFiles:
            ...
            cv.imshow(fname, self.signImgs[-1])  # [-1] : 배열 제일 마지막 값. 방금 전에 append 한 이미지
    ...
    def recognitionFunction(self):
        if self.roadImg is None:
            ...
            if len(GM[best]) < 4:  # 최선의 번호판이 매칭 쌍 4개 미만이면 실패
                ...
                points1 = np.float32([sign_kp[gm.queryIdx].pt for gm in good_match])  # 왼쪽 이미지
                points2 = np.float32([road_kp[gm.trainIdx].pt for gm in good_match])  # 오른쪽 이미지
                ...
                box2 = cv.perspectiveTransform(box1, H)  # H 에 의해서 임의의 사각형으로 바뀜
