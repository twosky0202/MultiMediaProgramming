...
mask = np.zeros((img.shape[0], img.shape[1]), np.uint8)  # 이미지 크기와 동일한 크기의 배열을 만들고 0으로 초기화
mask[:, :] = cv.GC_PR_BGD  # 모든 화소를 '배경일 것 같음'으로 초기화
...
background = np.zeros((1, 65), np.float64)  # 크기가 64인 배경 히스토그램 0으로 초기화
foreground = np.zeros((1, 65), np.float64)  # 크기가 64인 물체 히스토그램 0으로 초기화
